    ...
    for(let i=0;i<arr.length;i++)
    {
        ...
        if(arr[i].yearsOfExperience>1){
            if(arr[i].lastName!=null)
                candidant={fullName:`${arr[i].firstName} ${arr[i].lastName}`,tech:arr[i].tech};
            else
                candidant={fullName:`${arr[i].firstName}`,tech:arr[i].tech};
            newarr.push(candidant);
        }
    }
    return newarr;
};
...
const applicationsStatics = (arr) => {
    ...
    for(let i=0;i<arr.length;i++){
        if(arr[i].tech=='Python' && arr[i].yearsOfExperience>1)
            result.python_devs+=1;
        else if(arr[i].tech=='JS' && arr[i].yearsOfExperience>1)
            result.javaScript_devs+=1;
        else if(arr[i].tech=='.Net' && arr[i].yearsOfExperience>1)
            result.dotNet_devs+=1;
        else if(arr[i].tech=='Java' && arr[i].yearsOfExperience>1)
            result.java_devs+=1;
        if((arr[i].firstName==null && arr[i].lastName==null) || (arr[i].firstName=='' && arr[i].lastName=='') || arr[i].yearsOfExperience<=1)
...
const classesAvg = (data) => {
    ...
    for(let i=0;i<data.grades.length;i++)
    {
        for(let j=0;j<data.grades[i].classes.length;j++)
        {
            ...
            data.grades[i].classes[j].avg=Math.floor(data.grades[i].classes[j].avg);
        }
    }
    return data;
};
